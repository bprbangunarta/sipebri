...
- Jun 2026 (20/08): Referensi baru grup **Agunan**: `/collateral-types` (19 jenis agunan), `/collateral-binding` (7 jenis pengikatan — sebelumnya `/binding-types`), `/collateral-conditions` (6 kondisi agunan), `/collateral-methods` (4 metode hitung). Semua memakai pola `ReferenceController` + `pages/Reference.vue`; izin `collateral-*.view/manage` (total 36 izin), MenuSeeder membuat grup `Agunan`, breadcrumb ditambahkan di `resources/js/config/navigation.js`.
