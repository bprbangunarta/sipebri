...
- Jun 2026 (20/08): Dashboard = **ringkasan pemberian kredit** DATA STATIS dengan **filter kantor** (Semua Kantor + 7 kantor; angka diturunkan dari porsi kantor, kartu "Penyaluran per Kantor" hanya muncul pada mode Semua Kantor, antrean komite tersaring per kantor). Isi: 5 KPI (Pengajuan/Disetujui/Dicairkan/Menunggu Komite/SLA), Pengajuan vs Realisasi 10 bulan, Capaian Target + Hasil Keputusan Komite, Berkas per 9 Tahap, Komposisi Produk, Antrean Keputusan Komite, Aktivitas Terakhir (data nyata). Kartu Penyimpanan, Galeri Komponen, dan metrik NPL/kolektibilitas DIHAPUS (bukan lingkup origination). Ikon menu Jenis Pengikatan di MenuSeeder = `lock`.
