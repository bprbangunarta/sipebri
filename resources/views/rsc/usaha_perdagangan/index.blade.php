                    ...
                    @forelse ($perdagangan as $item)
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="{{ route('rsc.index.usaha.perdagangan.identitas', ['kode_usaha' => $item->kd_usaha, 'kode' => $data->kode, 'rsc' => $data->rsc, 'status_rsc' => $data->status_rsc]) }}"
                                    class="btn btn-sm btn-warning" style="float: left" title="Input Analisa">
                                    <i class="fa fa-file-text-o"></i></a>

                                <form
                                    action="{{ route('rsc.usaha.perdagangan.delete', ['kode_usaha' => $item->kd_usaha]) }}"
                                    method="POST">
                                    @method('delete')
                                    @csrf
                                    @method('delete')
    ...
    <div class="modal fade" id="modal-tambah">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('rsc.simpan.usaha.perdagangan', ['rsc' => $data->rsc]) }}" method="POST">
                    @csrf
