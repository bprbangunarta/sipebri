...
class PendampingController extends Controller
{
    public function edit(Request $request)
    {
        ...
        try {
            ...
            if (count($data_pengajuan) > 1 && $pendamping[0]->nama_pendamping == '' || is_null($pendamping[0]->nama_pendamping)) {
                $data_pengajuan = Pengajuan::where('nasabah_kode', $cek->kode_nasabah)->orderBy('created_at', 'desc')->skip(1)->first();
                $data_pendamping = Pendamping::where('pengajuan_kode', $data_pengajuan->kode_pengajuan)->first();
